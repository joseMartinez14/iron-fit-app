...
export default function NewPaymentClient({ clients }: { clients: ClientOption[] }) {
    ...
    const handleSave = async () => {
        ...
        if (validUntil <= paymentDate) {
            alert('La fecha de vigencia debe ser posterior a la fecha de pago');
            return;
        }
        ...
        try {
            ...
            await axios.post('/api/protected/payments', payload).then(async res => {
                setIsSaving(false);
                const clientName =
                    res.data?.payment?.client?.name || selectedClient?.name || 'cliente';
                setSuccessMessage(`Pago guardado con éxito para ${clientName}. Redirigiendo…`);
                ...
                alert('Hubo un problema al guardar el pago. Inténtalo nuevamente.');
            });
                        ...
                        {selectedClient && (
                            <div className="mt-2 p-3 bg-blue-50 rounded border">
                                ...
                                <div className="text-xs text-blue-700 mt-1">
                                    Último pago: {selectedClient.lastPaymentDate ? new Date(selectedClient.lastPaymentDate).toLocaleDateString() : '—'}
                                    {' • '}Válido hasta: {selectedClient.lastValidUntil ? new Date(selectedClient.lastValidUntil).toLocaleDateString() : '—'}
                                </div>
                            </div>
                        )}
                    </div>
